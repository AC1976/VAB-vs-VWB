...
if run_button:
    ...
    with tab2:
        ...
        if tax_model != TaxModel.NONE:
            ...
            median_growth = max(0, median_final + median_taxes - total_contributed)
            
            p10_final = np.percentile(simulator.final_values, 10)
            p10_taxes = np.percentile(simulator.total_taxes_paid, 10)
            p10_growth = max(0, p10_final + p10_taxes - total_contributed)
            
            p90_final = np.percentile(simulator.final_values, 90)
            p90_taxes = np.percentile(simulator.total_taxes_paid, 90)
            p90_growth = max(0, p90_final + p90_taxes - total_contributed)
            
            # Debug: Show the actual numbers
            st.write(f"**Debug Info:**")
            st.write(f"Total Contributed: ${total_contributed:,.0f}")
            st.write(f"Median: Final=${median_final:,.0f}, Taxes=${median_taxes:,.0f}, Growth=${median_growth:,.0f}")
            
            # Create THREE visualizations: stacked bar, absolute taxes, pie chart
            fig4 = plt.figure(figsize=(16, 5))
            gs = fig4.add_gridspec(1, 3, hspace=0.3, wspace=0.3)
            ax4 = fig4.add_subplot(gs[0, 0])
            ax5 = fig4.add_subplot(gs[0, 1])
            ax6 = fig4.add_subplot(gs[0, 2])
            
            # Chart 1: Stacked bar chart - Where Does Your Money Go
            ...
            bars1 = ax4.bar(x, contributions, width, label='Contributions', color='#2E86AB', 
                           edgecolor='black', linewidth=1)
            bars2 = ax4.bar(x, growth, width, bottom=contributions, label='Growth', color='#06A77D',
                           edgecolor='black', linewidth=1)
            bars3 = ax4.bar(x, taxes, width, bottom=np.array(contributions) + np.array(growth),
                           label='Taxes', color='#D62828', edgecolor='black', linewidth=1)
            
            # Add value labels
            for i in range(len(scenarios)):
                # Label for contributions
                ax4.text(i, contributions[i]/2, f'${contributions[i]/1e6:.1f}M', 
                        ha='center', va='center', fontweight='bold', fontsize=10, color='white')
                # Label for growth
                if growth[i] > 0:
                    ax4.text(i, contributions[i] + growth[i]/2, f'${growth[i]/1e6:.1f}M',
                            ha='center', va='center', fontweight='bold', fontsize=10, color='white')
                # Label for taxes
                if taxes[i] > 0:
                    ax4.text(i, contributions[i] + growth[i] + taxes[i]/2, f'${taxes[i]/1e6:.1f}M',
                            ha='center', va='center', fontweight='bold', fontsize=10, color='white')
            ...
            # Chart 2: Absolute Tax Amounts by Percentile
            tax_amounts = [p10_taxes, median_taxes, p90_taxes]
            
            bars = ax5.bar(scenarios, tax_amounts, width=0.6, 
                          color=['#FF6B6B', '#FF8C42', '#DC2F02'],
                          edgecolor='black', linewidth=1.5)
            
            # Add dollar labels on bars
            for bar, amount in zip(bars, tax_amounts):
                height = bar.get_height()
                ax5.text(bar.get_x() + bar.get_width()/2., height + height*0.02,
                        f'${amount:,.0f}', ha='center', va='bottom',
                        fontweight='bold', fontsize=11)
            
            ax5.set_ylabel('Total Taxes Paid ($)', fontsize=12)
            ax5.set_title('Absolute Tax Burden by Scenario', fontsize=13, fontweight='bold')
            ax5.grid(True, alpha=0.3, axis='y')
            ax5.yaxis.set_major_formatter(plt.FuncFormatter(
                lambda x, p: f'${x/1e6:.1f}M' if x >= 1e6 else f'${x/1e3:.0f}K'
            ))
            
            # Chart 3: Pie Chart - Government vs Your Share (Median Scenario)
            median_pre_tax = median_final + median_taxes
            slices = [median_taxes, median_final]
            colors = ['#D62828', '#06A77D']
            labels = [f'Government\n${median_taxes:,.0f}\n({median_taxes/median_pre_tax*100:.1f}%)',
                     f'You Keep\n${median_final:,.0f}\n({median_final/median_pre_tax*100:.1f}%)']
            
            wedges, texts = ax6.pie(slices, labels=labels, colors=colors, startangle=90,
                                    textprops={'fontsize': 11, 'fontweight': 'bold'})
            
            ax6.set_title(f'Your Money Split (Median)\nPre-Tax Value: ${median_pre_tax:,.0f}', 
